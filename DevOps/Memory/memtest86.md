...
There are many ways to test a computer's main memory. Naively, you can think of
memory as a binary object. While in binary systems, such as today's computers,
the state of a particular memory cell should be binary. It is not true that the
overall device state (the memory module) is OK or Not OK (NG), so it can
__not__be considered binary. There are many states in between OK and Not OK.
While in many circumstances memory appears to be OK and is considered to be
working, it can be seen, not only in supercomputing, that memory as an instance
of physics is not binary and there are several levels of OK, depending on usage
and user acceptance. For some unknown reason, ECC memory is not considered a
mainstream object of use.  Therefore, some errors are not detected and
therefore accepted by the user, even though the user may not admit it. In
supercomputers, memory that throws errors is less welcome. Usually ECC is used
where possible, but even then the memory needs to be validated. Unfortunately,
memory testing and evaluation is a difficult and time-consuming matter. The
reason for this is that certain value streams written to memory can cause an
error while others do not. For example, `11->00->11` might be a working pattern
without error, while `11->01->10->11` might not. This is because in certain
memory modules for example cross talk between memory lane is possible or some
other kind of local phenomena.
